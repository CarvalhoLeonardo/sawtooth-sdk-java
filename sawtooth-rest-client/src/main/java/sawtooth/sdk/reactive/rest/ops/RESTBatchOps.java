...
// @formatter:on
  ...
  public Future<Response> submitBatches(List<Batch> batches, MediaType mediaType) {
    ...
    WebTarget thisTarget = webTarget.path(REQPATH);
    Invocation.Builder thisBuilder = thisTarget.request();
    BatchList restPayload = rbl.build();
    return CompletableFuture.supplyAsync(() -> thisBuilder
        .accept(mediaType == null ? MediaType.APPLICATION_OCTET_STREAM : mediaType.toString())
        .post(Entity.entity(restPayload.toByteArray(),
            mediaType == null ? MediaType.APPLICATION_OCTET_STREAM : mediaType.toString())));
  }
