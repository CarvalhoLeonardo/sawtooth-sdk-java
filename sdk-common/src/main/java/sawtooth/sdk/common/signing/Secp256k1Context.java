...
package sawtooth.sdk.common.signing;
...
public class Secp256k1Context implements Context {
  ...
  private static byte[] generateCompactSig(final ECKey privateKey, final byte[] data) {
    ...
    System.arraycopy(Utils.bigIntegerToBytes(sig.r, HALF_NUM_SIGNATURE_BYTES), 0, csig, 0,
        HALF_NUM_SIGNATURE_BYTES);
  ...
  @Override
  public final boolean verify(final String signature, final byte[] data,
    ...
    byte[] sbytes = Arrays.copyOfRange(signatureBytes, HALF_NUM_SIGNATURE_BYTES,
        NUM_SIGNATURE_BYTES);
